...
def load_gui(self):
    ...
    self.lbl_fname = tk.Label(self.master, text='First Name: ')     #Defines the Label
    self.lbl_fname.grid(row=0, column=0, padx=(27,0), pady=(10,0), sticky=N+W)      #Determines label position with grid()
    ...
    #Create the inputs
    self.txt_fname = tk.Entry(self.master, text='')     #Defines the Entry(input)
    self.txt_fname.grid(row=1, column=0, rowspan=1, columnspan=2, padx=(30,40), pady=(0,0), sticky=N+E+W)        #Determines the Entry position with grid()
    ...
    #Define buttons and position them
    ...
    self.btn_close = tk.Button(self.master,width=12,height=2,text='Quit',command=lambda: phonebook_func.ask_quit(self))
