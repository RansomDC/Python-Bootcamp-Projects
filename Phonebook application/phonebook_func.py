...
def center_window(self, w, h): #pass in the tkinter frame(master) reference and the w(width) and h(height)
    #get user's screen width and eheight
...
def addToList(self):
    ...
    if (len(var_fname) > 0) and (len(var_lname) > 0) and (len(var_phone) > 0) and (len(var_email) > 0): #enforce the user to provide all parameters
        conn = sqlite3.connect('phonebook.db')
        with conn:
            cursor = conn.cursor()
            ...
            if chkName == 0: #if this is 0 then there is no existance of the fullname and we can add new data
                ...
                onClear(self) #call the function to clear all o fthe textboxes
        conn.commit()
        conn.close
    else:
